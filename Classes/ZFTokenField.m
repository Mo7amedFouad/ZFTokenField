...
- (void)reloadData
{
    ...
    self.textField.frame = (CGRect) {0, 0, self.bounds.size.width, [self.dataSource lineHeightForTokenInField:self]};
...
{
    [self scrollRectToVisible:self.textField.frame animated:YES];
}
...
- (void)enumerateItemRectsUsingBlock:(void (^)(CGRect itemRect))block
{
    ...
    if ([self.tokenDelegate respondsToSelector:@selector(tokenMarginInTokenInField:)]) {
        margin = [self.tokenDelegate tokenMarginInTokenInField:self];
    }
...
- (void)textFieldDidBeginEditing:(ZFTokenTextField *)textField
{
    ...
    if ([self.tokenDelegate respondsToSelector:@selector(tokenFieldDidBeginEditing:)]) {
        [self.tokenDelegate tokenFieldDidBeginEditing:self];
    }
}

- (BOOL)textFieldShouldEndEditing:(ZFTokenTextField *)textField
{
    if ([self.tokenDelegate respondsToSelector:@selector(tokenFieldShouldEndEditing:)]) {
        return [self.tokenDelegate tokenFieldShouldEndEditing:self];
    }
    return YES;
}
...
- (void)textFieldDidChange:(ZFTokenTextField *)textField
{
    if ([[textField rawText] isEqualToString:@""]) {
        ...
        if ([self.tempTextFieldText isEqualToString:@"\u200B"]) {
            if (self.tokenViews.count > 1) {
                ...
                if ([self.tokenDelegate respondsToSelector:@selector(tokenField:didRemoveTokenAtIndex:)]) {
                    [self.tokenDelegate tokenField:self didRemoveTokenAtIndex:removeIndex];
                }
            }
        }
    }
    ...
    if ([self.tokenDelegate respondsToSelector:@selector(tokenField:didTextChanged:)]) {
        [self.tokenDelegate tokenField:self didTextChanged:textField.text];
    }
}

- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if ([self.tokenDelegate respondsToSelector:@selector(tokenField:didReturnWithText:)]) {
        [self.tokenDelegate tokenField:self didReturnWithText:textField.text];
    }
    return YES;
}
